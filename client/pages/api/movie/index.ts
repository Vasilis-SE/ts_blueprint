...
const getHandler = async (): Promise<ISuccessfulResponseData | IFailedResponse> => {
    return await Fetch.get(`${process.env.API_BASE_URL}/movie?page=0&limit=${process.env.RESULTS_LIMIT}&order=${process.env.MOVIES_ORDER_BY}&sort=${process.env.MOVIES_ORDER_METHOD}`);
}