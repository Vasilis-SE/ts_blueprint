...
class MovieService {
    ...
    getMoviesMeta(baseUrl, resultsNum, params, query) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                let _prev = "";
                let _next = "";
                let queryClonePrev = Object.assign({}, query);
                let queryCloneNext = Object.assign({}, query);
                // Create filters for the query
                const finalFilters = yield this._getMovieFilters(query);
                if (('range' in params) && params.range != '') {
                    if (params.range.indexOf(',') === -1)
                ...
                const _model = new movie_3.default(params);
                const moviesCount = yield _model.reportMoviesCount(finalFilters);
                if (!moviesCount)
                    throw new Error();
                let page = ('page' in query) && query.page > 0 ? query.page : 0;
                let limit = ('limit' in query) && query.limit > 0
                    ? query.limit
                    : movie_2.MovieGlobals.QUERY_LENGTH;
                const offset = (page * limit) + resultsNum;
                let _pages = Math.ceil(Number(moviesCount) / limit);
                if (page) {
                    queryClonePrev.page--;
